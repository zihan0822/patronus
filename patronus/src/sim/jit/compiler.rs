    ...
    fn mock_interpret(&mut self) -> Value {
        ...
        while let Some((e, args_available)) = todo.pop() {
            if let Some(&ret) = cached.get(&e) {
                // heap allocated bv
                match e.get_type(self.expr_ctx) {
                    expr::Type::BV(width) if width > 64 && expr_nodes[&e].num_references > 1 => {
                        value_stack.push(self.clone_bv(ret, width))
                    }
                    _ => value_stack.push(ret),
                }
                continue;
            }
